...
bind-key C-t run-shell -b "cat $HOME/.history.icd | fzf-tmux -- --header '[Create session]' | xargs -0 bash -c 'source ~/.config/zsh/tools/tmux.zsh && tmux display \"Session created for $@\" && ts_from_arg \"$@\"' - "
